<template>
  <div class="space-y-10 divide-y divide-gray-900/10 dark:divide-gray-50 dark:bg-primary p-10 lg:h-screen sm:h-max">
      ...
      <form class="grid grid-cols-1 gap-x-8 gap-y-8 pt-14 md:grid-cols-3 h-max" @submit.prevent="createStudent">
          <div class="bg-white shadow-sm ring-1 ring-gray-900/25 sm:rounded-xl md:col-span-3 dark:bg-slate-800">
              <div class="grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6 px-4 py-6 sm:p-8">
                  ...
                  <div class="sm:col-span-3">
                      ...
                      <div class="mt-2">
                          <input type="text" name="first-name" id="first-name" autocomplete="given-name" placeholder="Enter first name" v-model="student.firstname"
                  ...
                  <div class="sm:col-span-3">
                      ...
                      <div class="mt-2">
                          <input type="text" name="last-name" id="last-name" autocomplete="family-name" placeholder="Enter last name" v-model="student.lastname"
                              class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 sm:text-sm sm:leading-6 box-border p-2 dark:bg-transparent dark:text-white" />
                      </div>
                  </div>
          ...
          <div class="flex items-center justify-end gap-x-3 px-4 py-4 sm:px-8 col-span-full">
              ...
              <button type="submit" class="w-28 bg-primary text-white rounded-sm text-sm px-4 py-3 dark:bg-sky-500 dark:hover:bg-sky-400">Add</button>
          </div>
      </form>
  </div>
</template>
...
import { Create } from '@/components/api/admin/create'
...
async function createStudent() {
  ...
  const response = await Create.createStudent(params)
  alert('Student records has been successfully added.')
}
